...
\alexey{I like this section, but I wonder if ``types of knowledge'' is a good way to describe what we have here - it sounds too broad. Perhaps ``types of information'' or perhaps these are even (knowledge manipulation) actions?}
\dmg{I think it is too much of a change. the emphasis of the text has been on knowledge, so let us leave it}
...
\vspace{-3mm}
    ...
    question and directly cites or links to another answer in the thread, or when a
    user cites another question or answer in a comment (a typical case is linking to a previously asked question).
    Figure \ref{fig:SO-PK1} depicts an example of participatory knowledge in Stack Overflow: the
    author participate in the comments to help another user when the answer is not sufficient for a
    particular case.  In the comment, the user references another authors and answer.
...
\item[Support follow-up questions:] It is not uncommon to use the mailing list to have follow-up discussions on specific \SO
  questions. These discussions were more specific about the rational for an answer.\dmg{here I am speculating this is what carlos meant}\\
  \gpoo{Consider that users post in both \channels simultaneously. Although there would be interest (or urgency) in having the question answered, in \SO the cycle ends there (if there is more questions, there won't be immediate connection with the participants). However, in the \ml the discussion can continue with follow-up questions from anybody (not only from the person who asked the question), it can even divert to something else based on one of the points made during the discussion. Therefore, people can learn more (not only the rationale of an answer), but also similar topics that arise during follow-up questions.
  [If I could make it shorter, I would have edited the text directly. But at least that is the idea.]}
