...
using different communication channels, and discussing how channel
...
<<<<<<< 16bce47613ca036a57cb2ca31cb51be5fd4de329
...
is sought, shared, and curated on both the \RH mailing list and the R-
...
=======
As mentioned, the main goal of this work is to empirically compare how knowledge, specifically knowledge in question-and-answer (Q\&A) form, is sought, shared, and curated in both the \RH mailing list and the R tag on \SO. We used a mixed methods \textit{exploratory case study} methodology~\cite{Creswell2009,Runeson2012} to answer our research questions (presented in Section~\ref{cha:introduction}).
>>>>>>> revert the first half of methodology to previous version, and create a new phase for the extension
...
\subsection{Phase III: A Deeper Quantitative Analysis}

\alexey{the following is not written yet, I just put here the parts that mention the extension phase}
...
number of email addresses from 36.6k to 31.7 persons=--a reduction of 15\%; a similar number was reported in 
\cite{Vasilescu2014c}).

To determine which users were active in both channels, we compared the
MD5 hashes of the email addresses of \RH mailing list and \SO participants. If there was
match, we associated the \SO user to the same person. This way we
identified 1,449 persons who have used both media channels.

To determine which users were active in both channels, we compared the MD5 hashes of the email addresses extracted from the \RH mailing list and \SO. If there was match, we associated the \RH and \SO user accounts to the same person. Using this approach, we identified 1,449 persons who used both media channels.

  \begin{table}[!htb]
    \centering
      \caption{Raw data collected for each channel during Phase I.}
      \begin{small}
        \begin{tabular}{lrr}
          \toprule
          Type          &  \RH & \SO \\
          ...
          Answers       & 213,366 &  99,620 \\
          Comments      &       - & 286,124 \\
          Different individuals\tablefootnote{Up to September 2014.} & 31,729 &  26,324 \\
          \bottomrule
        \end{tabular}
      \end{small}
    \label{table:data}
