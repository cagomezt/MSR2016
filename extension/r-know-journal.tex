...
In this paper, we report a qualitative and quantitative study of how one community of practice—the R software development community—creates and curates knowledge associated with questions and answers (Q\&A) in two of its main \channels: the R-tag in Stack Overflow and the R-users mailing list. The results reveal that knowledge is created and curated in two main forms: participatory, where multiple members explicitly collaborate to build knowledge, and crowdsourced, where individuals work independently of each other. \alexey{I think we should add a sentence here about participation over time, or about prolific users. Will do that later.} The contribution of this paper are: a characterization of knowledge types that are exchanged by these communities of practice, including a description of the reasons why members choose one channel over the other; and a qualitative study of the evolution of participation in both channels. Finally, this paper enumerates a set of recommendations to assist practitioners in the use of multiple channels for Q\&A.
\end{abstract}
