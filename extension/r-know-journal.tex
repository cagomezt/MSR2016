...
\setboolean{showcomments}{true} \ifthenelse{\boolean{showcomments}}
  {
				\newcommand{\nbb}[2]{
...
\newcommand{\rqd}{How has the community of both Stack Overflow and the R-help mailing list evolved over time?}
...
\alexey{we'll need to add here the mention of the extension findings: the participation behavior on both channels, in terms of \textbf{quantity} and \textbf{continuity}.}
...
\begin{enumerate}[label=\bfseries{RQ\arabic*.},itemsep=3pt, topsep=2pt, leftmargin=3em, parsep=0pt]
        ...
        \item \rqd
    \end{enumerate}
\alexey{Add research questions regarding the behavior of new and prolific contributors?}

By mining archival data, we identified and categorized the main types of knowledge artifacts contained on the \RH mailing list and in \SO messages (RQ1). The emerging categories (see Table~\ref{table:type-of-knowledge}) form a \textit{typology} that allows researchers to study and characterize Q\&A knowledge dissemination within a community of practice. We used the typology to study how knowledge is constructed and shared on \SO and the \RH mailing list. We found that these channels support two distinct approaches for constructing knowledge---\textit{participatory knowledge construction} and \textit{crowd knowledge construction}---however, each channel supports them differently (RQ2). Our findings indicate that participatory knowledge construction is more prevalent on \RH, while crowd knowledge construction is more prevalent on \SO.
...
\alexey{and here we need to bring the extended results Daniel has found for the extension.}
...
\alexey{do we present all the findings as a single study or as a study and then an extension? if we present it as a single study, we need to adjust the methodology.}
...
\textbf{this needs to be moved to methodlogy/data gathering}

In order to make the comparison more meaningful, we unified email addresses of the same persons (this step
reduced the number of emails from 36.6k to 31.5k---a reduction of 15\% the same reduction reported in
\cite{Vasilescu2014c}).
...
2013~\cite{Vasilescu2014c}. Figure~\ref{fig:channelsActPerMonth} shows the activity of questions per month in both channels up to Sept 2016. We
have also included \SO questions with a positive score (the sum of positive minus negative votes is larger than zero). As it can be
seen, \SO has continued its growth while \RH has decreased significantly to a steady count. However, in the last year, total number of
questions in both channels seem to
have stabilized. Figure~\ref{fig:channelsActPerMonthLatests} shows only the number of questions since Jan 2015. At it can be
seen, both channels are relatively flat in overall questions. The number of questions in \SO is between 10 and 20 times
the number of questions in \RH. However, the number of positive questions in \SO is starting to
decrease. 
...
\subsubsection{Regarding users}

The number of participants of each channel is

An important aspect of a community is its ability to attract new participants and to make them stay. We looked at the
number of participants that only ask one question and do not have any other contributions (either questions of responses).

\subsubsection{Common users of both channels}

The number of common users between both datasets is relatively small (around 2.5\%). Yet, a handful of these
contributors are responsible for a very large proportion of the answers to both channels. Figure~\ref{fig:common} shows
the accumulated proportion of answers that have been contributed by these authors. As it can be seen, the top
contributor has contributed 3.9\% and 3.7\% of answers in \RH and \SO, respectively. In fact, the top 6 contributors to
both channels are responsible of 10\% of the answers to both channels. Answers to \SO include both posts and comments to questions.
...
\subsubsection{Prolific users}


Both channels benefit from few very prolific users. Figure~\ref{fig:prolific} shows the most prolific users  in each
channel, ordered by the number of answers they have contributed. The vertical axis is the the accumulated proportion of
contributions to all answers in each channel. In \RH, 9 users have contributed 25\% of all answers, and 49 50\%. In \SO,
the distribution is not as steep, yet, 24 users have contributed 25\% of all answers (including comments) and 131 are
have contributed 50\%. This means that 0.15\% of the users of \RH contribute 50\% of all the answers, while 0.45\% of
users of \SO
contribute 50\% of all the answers.
...
New users are important for the survival and continuity of a community. Figure~\ref{fig:newusers} shows the proportion
of users who have posted their first question in a particular month (thicker lines). As it can be seen, both communities
show a very similar pattern: between 25 and 35\% of its users who post new questions are new during that month. However,
there is a significant subset of these users: those who ask one question and never contribute again (either questions or
answers). These users are represented by the thinner lines. In \SO around 1 in 10 users only ever post one question,
while in \RH the number has been increasing over time. There has been a visible decrease in the proportion of new
users in both channels.

In fact, the number of users who only participate once is relatively large over the lifetime of the channels: 43\% in \RH and and 32\% in
\SO.
...
\begin{figure*}[htbp]
  ...
  \caption{Proportion of new users over time. The top lines (thicker)x correspond to
...
To measure the length of participation in either
channel we divided the history of both channels into one month segments. In \RH a user participate in a given month if she
posted an email to the list during that period. For \SO, we considered a user participated in a given month if she
posted a question, a response or a comment.

The results show that users do not continue participating in either channel for a long
time. Figure~\ref{fig:monthsPerUser} shows the accumulated proportion of users that participate a given number of
months. The curves are very similar and skewed: 62\% of \RH users and 65\% of \SO users are active one month only and
...
\begin{figure*}[htbp]
  ...
  \caption{Months a user has been active. This plot shows the accumulated proportion of users who have been active at a
    ...
    90\% of \RH users are active 5 or less months. In SO, 90\% of users participate 4 or less months.}
...
{
  \color{red}{\textbf{this section needs to be written}}
  }
