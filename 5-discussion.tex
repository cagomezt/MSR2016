...
In contrast, on Stack Overflow questions that trigger a discussion are flagged as opinion-based, or as off-topic, and they are likely to be closed. 
...
One of the main benefits enabled by \SO's crowd-based knowledge construction is the creation and curation of a questions and answers pool. In contrast, \RH provides an environment in which users
...
While \SO  has been successful, some users feel that by not fostering discussion, it restricts thinking that might lead towards better answers. 
For instance, P26 explains:
    ...
    \end{quote}
    Similarly, P35 explained that they use the R-help mailing list if the questions are not 100\% \textit{``help-me-to-code-this''}.

    In contrast, \SO shines when questions have to be kept for posterity. 
    The curation mechanisms provide tools to keep the channel clean of what seems to be unnecessary information (e.g., flagging questions, deleting comments, editing messages, and demoting irrelevant answers). As P14 explains:
...
An important research question that arises from these findings is whether \SO's model can be improved to provide better participatory knowledge construction support without hindering its ability to curate it for future use.

Another interesting aspect emerging from our findings, is that the activity on \RH mailing list is only marginally smaller than on \SO (the proportion of responses in each category fluctuated between 1.4 and 2 times only). Further research is required to verify the quality and effectiveness of answers.

\alexey{this part needs to be a bit more extended, but could be an interesting section.}
