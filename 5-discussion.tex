...
\label{cha:theory}
    ...
    way knowledge is shared on both channels, and the recommendations of the use of Q\&A \channels.
...
An example of difference between communities is the question \textit{``Arrange elements on a matrix according to rowSums + short `apply' Q''}  posted to  Stack Overflow\footnote{\url{http://goo.gl/a8AES8}} and {R-help}\footnote{\url{http://goo.gl/PGflT5}}.
    Each community answered the question using a different knowledge construction approach.
    On Stack Overflow, each participant provided its own solution without any evidence of collaboration.
...
\subsection{Recommendations for using multiple Q\&A \channels}
    ...
    One of the expected outcomes of this study was to derive a set of recommendations for using different \channels.
...
\subsubsection{\recb}
    ...
    Throughout this study, we noticed that most of the harsh responses were given to users who neither read the posting guide nor had learned the basic concept of R or statistics.
    For anybody using a channel, the community expects that they should learn about it in advance, and learn the basics of the technology that they are using.
