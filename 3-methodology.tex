...
For inter-rater reliability, we used the Cohen Kappa \textbf{inter-rater agreement} coefficient~\cite{Stemler2004}. Although it is suggested to aim for coefficient values above 0.6 to obtain substantial results~\cite{Landis1977}, based on our previous experience \cassie{Reference?}with this method, we aimed for 0.8 or above. We used this coefficient after each coding session as a way to trigger discussion.
...
For mapping the R-help mailing list messages, we defined the following:\cassie{I might be misunderstanding the context, but why don't we talk about mapping the Rtag stuff on SO too?}
